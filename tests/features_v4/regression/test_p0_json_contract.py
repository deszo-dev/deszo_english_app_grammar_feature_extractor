...
from grammar_feature_extractor._internal.models import NPFeature, PredicateFeature
...
def _sentence(text: str, words: list[dict[str, object]]):
    ...
    return GrammarFeatureExtractor().extract_page(document).features[0]


def _np_by_head(sentence, head_ref: int) -> NPFeature:
    for np in sentence.features.syntax.np_profiles:
        if np.head == head_ref:
            return np
    raise AssertionError(f"NP with head_ref={head_ref} not found")


def _predicate(sentence) -> PredicateFeature:
    return sentence.features.syntax.predicates[0]
    ...
    dogs_np = _np_by_head(
        _sentence(
            ...
            [
                ...
                _word(
                    "need", "need", "VERB", 0, "root", 5, 9, "Tense=Pres|VerbForm=Fin"
                ),
...
def test_fx_024_fx_025_fx_026_strict_predicate_types() -> None:
    ...
    passive = _predicate(
        _sentence(
            ...
            [
                ...
                _word(
                    "was", "be", "AUX", 4, "aux:pass", 9, 12, "Tense=Past|VerbForm=Fin"
                ),
    ...
    active = _predicate(
        _sentence(
            ...
            [
                ...
                _word(
                    "wrote", "write", "VERB", 0, "root", 4, 9, "Tense=Past|VerbForm=Fin"
                ),
    ...
    assert nominal.predicate_type == "copular_nominal"
    assert passive.predicate_type == "passive_verbal"
    assert passive.voice == "passive"
    assert active.predicate_type == "verbal"
